  ...
  return Object.entries(LOS).map(([topic, {
    ...
    moduleNames.forEach(m => {
      ...
      moduleStats[m] = ms.length ? {
        pct: Math.round(ms.reduce((s, h) => s + (h.pct || 0), 0) / ms.length),
        sessions: ms.length,
        totalQs: ms.reduce((s, h) => s + (h.total || 0), 0)
      } : null;
    });
...
function qcAdd(cache, t, st, diff, questions) {
  ...
  const slots = [...(cache[k] || [])];
  const slot = {
    qs: questions.slice(0, 15),
    usedOn: null,
    ts: Date.now()
  };
  if (slots.length < QCACHE_SLOTS) {
    slots.push(slot);
  } else {
    const oldest = slots.reduce((mi, s, i) => s.ts < slots[mi].ts ? i : mi, 0);
    slots[oldest] = slot;
  }
  const updated = {
    ...cache,
  ...
  if (keys.length > QCACHE_MAX) {
    const sorted = keys.map(ky => ({
      ky,
      latest: Math.max(...(updated[ky] || []).map(s => s.ts || 0))
    })).sort((a, b) => a.latest - b.latest);
    sorted.slice(0, keys.length - QCACHE_MAX).forEach(({
      ky
    }) => delete updated[ky]);
  }
  return updated;
...
const POWER_NOTES = {
  ...
  "Financial Statement Analysis": {
    topics: [{
      ...
      rules: ["Revenue recognition (IFRS 15/ASC 606): 5-step model — identify contract, performance obligations, transaction price, allocate, recognise when (or as) obligation satisfied.", "Gross vs net revenue: gross = full selling price; net = after agent fees. Principal reports gross; agent reports net.", "EPS basic = (NI − Preferred Dividends) / Weighted Average Shares.", "EPS diluted: include convertible securities, options, warrants if dilutive (i.e. reduce EPS).", "Antidilutive securities (those that increase EPS) are EXCLUDED from diluted EPS.", "Comprehensive income = NI + Other Comprehensive Income (OCI). OCI includes FX translation, unrealised gains on AFS securities, pension adjustments.", "Non-recurring items: discontinued operations and extraordinary items (rare under IFRS) shown separately, net of tax."],
      traps: ["Diluted EPS ≤ Basic EPS always. If a security increases EPS, it is antidilutive — exclude it.", "OCI items bypass the income statement — they don't affect NI but do affect equity.", "Long-term contracts: percentage-of-completion (IFRS preferred) vs completed contract (GAAP allows for private).", "Bill-and-hold and channel stuffing are aggressive revenue recognition red flags."],
      mnemonic: "IPAT: Identify → Performance obligations → Allocate → Transfer (5-step simplified)."
    }, {
      module: "Cash Flow Statement",
      rules: ["Three sections: CFO (operations), CFI (investing — asset purchases/sales), CFF (financing — debt/equity).", "Indirect CFO: start with Net Income → add non-cash charges (depreciation, amortisation) → adjust working capital changes.", "Direct CFO: lists actual cash receipts and payments. Both methods produce the same CFO total.", "IFRS vs GAAP — interest paid: IFRS = CFO or CFF; US GAAP = CFO only.", "IFRS vs GAAP — dividends paid: IFRS = CFO or CFF; US GAAP = CFF only.", "IFRS vs GAAP — interest received: IFRS = CFO or CFI; US GAAP = CFO only.", "IFRS vs GAAP — dividends received: IFRS = CFO or CFI; US GAAP = CFO only.", "Cash flow linkage: NI links to CFO (indirect method starting point). WC changes link balance sheet to income statement.", "Free cash flow to firm: FCFF = CFO + Int(1−t) − CapEx."],
      traps: ["Depreciation is added back to NI in indirect CFO — it is non-cash, NOT a source of cash.", "Sale of PP&E = CFI inflow. Any gain on sale is in NI → must subtract from CFO in indirect method (avoid double-count).", "Rising NI but falling/negative CFO is a red flag for earnings manipulation.", "LIFO liquidation boosts NI and CFO artificially — inventory dips into older lower-cost layers.", "Capitalising vs expensing: capitalised costs → lower CFO, higher CFI outflow. Expensed → higher CFO outflow."],
      ...
      rules: ["LIFO (US GAAP only, not IFRS): rising prices → higher COGS → lower NI → lower taxes → higher cash flow.", "FIFO: rising prices → lower COGS → higher NI → higher taxes → lower ending inventory.", "LIFO reserve = FIFO inventory − LIFO inventory. Add to convert LIFO balance sheet to FIFO.", "COGS (FIFO) = COGS (LIFO) − ΔLIFO reserve.", "Capitalise cost if future economic benefit; expense if consumed in period.", "Depreciation methods: straight-line, double-declining balance (accelerated), units-of-production.", "Impairment: IFRS uses one-step (recoverable amount < carrying value); GAAP uses two-step.", "IFRS allows revaluation of PP&E to fair value; US GAAP does not (only downward impairment)."],
      traps: ["LIFO liquidation: selling old inventory layers → artificially low COGS → one-time NI boost. Not sustainable.", "Accelerated depreciation: higher depreciation early → lower NI early but HIGHER cash taxes paid later (DTL created).", "Intangible assets: internally developed (e.g. R&D) generally expensed; acquired intangibles capitalised.", "Research = always expense. Development = capitalise under IFRS if criteria met; expense under US GAAP."],
      ...
      rules: ["Deferred Tax Liability (DTL): accounting income > taxable income → pay LESS tax now, more later. Tax owed in future.", "Deferred Tax Asset (DTA): taxable income > accounting income → pay MORE tax now, benefit later.", "Common DTL cause: accelerated depreciation for tax (lower taxable income now → DTL).", "Common DTA causes: warranty expense accruals, pension expense, losses carried forward.", "Temporary differences → deferred taxes. Permanent differences (tax-exempt interest, fines) → NO deferred tax.", "Effective tax rate = Income Tax Expense / Pre-tax Income. May differ from statutory rate.", "Valuation allowance (US GAAP): reduces DTA if 'more likely than not' it cannot be realised.", "Tax base vs carrying value: DTA when tax base of asset > carrying value, or tax base of liability < carrying value."],
      traps: ["DTL ≠ a penalty — it is simply taxes deferred to a future period, a normal result of timing differences.", "DTA ≠ a tax refund — it represents future tax savings already paid or accrued.", "Only TEMPORARY differences create deferred taxes. Permanent differences never create DTA/DTL.", "A change in tax rate affects ALL existing DTA and DTL balances immediately in the period of change.", "If a DTA has a valuation allowance, net DTA on balance sheet is lower — signals uncertainty about future profits."],
  ...
  try {
    ...
    const res = await fetch(AI_PROXY_URL, {
      ...
      body: JSON.stringify({
        ...
        userId,
        messages: trimmed,
        maxTokens: Math.min(maxTokens, 450),
        level
      })
    });
...
function parsePNAIResponse(text, moduleName) {
  ...
  for (const line of lines) {
    if (/^RULES?:?\s*$/i.test(line)) {
      section = "rules";
      continue;
    }
    if (/^TRAPS?:?\s*$/i.test(line)) {
      section = "traps";
      continue;
    }
    if (/^MNEMONIC:?\s*$/i.test(line)) {
      section = "mnemonic";
      continue;
    }
    if (section === "rules" && /^[•\-\*]/.test(line)) rules.push(line.replace(/^[•\-\*]\s*/, ""));else if (section === "traps" && /^[•\-\*]/.test(line)) traps.push(line.replace(/^[•\-\*]\s*/, ""));else if (section === "mnemonic") mnemonic = (mnemonic ? mnemonic + " " : "") + line;
  }
  return {
    module: moduleName,
    rules: rules.length ? rules : ["Review the official CFA curriculum for this concept."],
    traps: traps.length ? traps : ["Verify distinctions carefully."],
...
function parseFormulaAIResponse(text, conceptName) {
  ...
  for (const line of lines) {
    ...
    if (/^FORMULA:\s*/i.test(line)) {
      f = line.replace(/^FORMULA:\s*/i, "").trim();
      section = "";
      continue;
    }
    ...
    if (/^WHEN:\s*/i.test(line)) {
      when = line.replace(/^WHEN:\s*/i, "").trim();
      section = "when";
      continue;
    }
    ...
    if (section === "variables") variables = (variables ? variables + "\n" : "") + line;else if (section === "when") when = (when ? when + " " : "") + line;else if (section === "example") example = (example ? example + " " : "") + line;
  }
  return {
    name,
    f: f || "See curriculum",
...
function RevisionScreen({
  ...
  const [dynamicFormulas, setDynamicFormulas] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem(DYNAMIC_FORMULAS_KEY) || "{}");
    } catch {
      return {};
    }
  });
  ...
  const generatePNForConcept = async card => {
    ...
    const reply = await callAIChat(userId, [{
      role: "user",
      content: prompt
    }], 450, cfaLevel);
    setPnGenerating(s => ({
  ...
  const generateFormulaForConcept = async card => {
    ...
    const updated = {
      ...dynamicFormulas,
      [selTopic]: [...existing, formula]
    };
    setDynamicFormulas(updated);
    try {
      localStorage.setItem(DYNAMIC_FORMULAS_KEY, JSON.stringify(updated));
    } catch {}
  };
  return /*#__PURE__*/React.createElement("div", {
    style: {
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      if (modIdx < 0) {
        ...
        return /*#__PURE__*/React.createElement("div", {
        ...
        }, conceptName || "Unknown concept"), /*#__PURE__*/React.createElement("div", {
          style: {
            display: "flex",
            flexWrap: "wrap",
            gap: "3px 5px"
        ...
        }, "×", c.wrongCount))))), /*#__PURE__*/React.createElement("span", {
          style: {
            fontSize: 10,
            background: "#e05070",
            color: "#fff",
        ...
        }, totalWrong, "×")), alreadyGenerated ? /*#__PURE__*/React.createElement("div", {
          style: {
            fontSize: 11,
            color: "#6060b0",
            fontStyle: "italic"
          }
        }, "✦ AI notes generated — see below") : /*#__PURE__*/React.createElement("button", {
          onClick: () => generatePNForConcept(cards[0]),
          ...
          style: {
            ...
            color: isGenerating ? "#6060b0" : C.accentLight,
            cursor: isGenerating || !userId ? "default" : "pointer",
            width: "100%",
            textAlign: "center"
          }
        }, isGenerating ? "⏳ Generating AI notes…" : "✨ Generate AI Study Notes", !userId && " (sign in required)"));
      }
      return /*#__PURE__*/ /* Clicking the card opens & scrolls to the module in the accordion below */React.createElement("button", {
        key: gi,
        onClick: () => {
      ...
      }, /*#__PURE__*/React.createElement(React.Fragment, null, "📚 ", matchedMod.module, isAuto && /*#__PURE__*/React.createElement("span", {
        style: {
          fontSize: 9,
          fontWeight: 600,
          color: "#6060b0",
      ...
      }, totalWrong, "×"), /*#__PURE__*/React.createElement("span", {
        style: {
          fontSize: 11,
          color: "#6060a0"
        }
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, mod.module, mod._aiGen && /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 9,
        fontWeight: 600,
        color: "#7c5cbf",
    ...
    }, mod.rules.length, " rules · ", mod.traps.length, " traps", mod.mnemonic ? " · 1 mnemonic" : "", mod._aiGen ? " · AI-generated from your mistakes" : "")), /*#__PURE__*/React.createElement("span", {
      style: {
        fontSize: 12,
        color: C.accentLight,
        fontWeight: 700,
      ...
      onClick: () => openAI(mod.module, `CFA Level ${cfaLevel} exam prep. I'm studying the "${mod.module}" module. Give me the 3 most important things to know for the exam about this topic, with a worked example for the trickiest one. Be concise.`),
      style: {
        marginTop: 14,
        width: "100%",
        padding: "9px",
    ...
    const gaps = wrongCards.filter(card => {
      const words = ((card.concept || "") + " " + (card.subtopic || "")).toLowerCase().split(/[\s\-\/\(\)]+/).filter(w => w.length > 3);
      const matched = formulaData.some(f => {
        const fn = f.name.toLowerCase();
        return words.some(w => fn.includes(w) || w.includes(fn.split(" ")[0]));
      });
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "✨ Formula Gaps from your mistakes"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        flexDirection: "column",
        gap: 6
      ...
      }, "No formula found — AI can generate one")), /*#__PURE__*/React.createElement("button", {
        onClick: () => generateFormulaForConcept(card),
        disabled: isGen || !userId,
        style: {
          padding: "6px 11px",
          borderRadius: 7,
          fontSize: 11,
          fontWeight: 700,
          border: "1px solid #8040c055",
          background: "#8040c018",
          color: isGen ? "#6060b0" : "#c090f0",
          cursor: isGen || !userId ? "default" : "pointer",
          flexShrink: 0,
          whiteSpace: "nowrap"
        }
      }, isGen ? "⏳ Generating…" : "✨ Generate"));
    })));
  })(), formulaData.length === 0 && !dynamicFormulas[selTopic]?.length ? /*#__PURE__*/React.createElement("div", {
    style: {
      textAlign: "center",
      padding: "40px 0",
      color: C.muted,
    ...
    style: {
      fontSize: 11,
      color: f._aiGen ? "#a060e0" : C.muted,
      minWidth: 110,
      flexShrink: 0,
      lineHeight: 1.4
    }
  }, f.name, f._aiGen && /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 9,
      color: "#7c5cbf",
      marginLeft: 4
  ...
  }, "Thinking…")), !userId && aiPanel.messages.length === 1 && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "10px 14px",
      borderRadius: 10,
      background: "#1a0a0e",
  ...
  }, "Sign in to enable AI-powered explanations.")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 8,
...
function CFAMock() {
  ...
  const [todayStarted, setTodayStarted] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("cfa_today_started") || "{}");
    } catch {
      return {};
    }
  });
  ...
  const [cfaLevel, setCfaLevel] = useState(() => {
    try {
      return localStorage.getItem(CFA_LEVEL_KEY) || "1";
    } catch {
      return "1";
    }
  });
  ...
  useEffect(() => {
    const recoverData = async () => {
      ...
      try {
        const qc = await storageGet(QCACHE_KEY);
        if (qc && typeof qc === "object") qCacheRef.current = qc;
      } catch {}
      setQdbLoaded(true);
  ...
  const generateFSAVignette = async (subtopic, difficulty) => {
    ...
    try {
      if (!authUser?.userId) {
        setError("FSA Vignette requires a ClearCFA account. Please sign in.");
        setLoading(false);
  ...
  const openReviewAI = async (context, firstPrompt) => {
    ...
    const withReply = [...reviewAiMsgsRef.current, {
      role: "assistant",
      content: reply || "No response — sign in and try again."
    }];
    reviewAiMsgsRef.current = withReply;
    setReviewAiPanel(p => p ? {
      ...p,
      messages: withReply
    } : null);
  ...
  const ReviewAIChatPanel = () => reviewAiPanel ? /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    onClick: () => setReviewAiPanel(null),
    style: {
      position: "fixed",
      inset: 0,
      background: "rgba(0,0,0,0.55)",
  ...
  }, reviewAiPanel.context)), /*#__PURE__*/React.createElement("button", {
    onClick: () => setReviewAiPanel(null),
    style: {
      background: "none",
      border: "none",
  ...
  }, reviewAiPanel.messages.map((m, i) => /*#__PURE__*/React.createElement("div", {
    key: i,
    style: {
      display: "flex",
      justifyContent: m.role === "user" ? "flex-end" : "flex-start"
  ...
  }, m.content))), reviewAiLoading && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      justifyContent: "flex-start"
    }
  ...
  }, "Thinking…")), !authUser?.userId && reviewAiPanel.messages.length === 1 && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "10px 14px",
      borderRadius: 10,
      background: "#1a0a0e",
    ...
    value: reviewAiInput,
    onChange: e => setReviewAiInput(e.target.value),
    onKeyDown: e => {
      if (e.key === "Enter" && !e.shiftKey) {
        e.preventDefault();
        sendReviewAI();
    ...
    onClick: sendReviewAI,
    disabled: reviewAiLoading || !reviewAiInput.trim(),
    style: {
      padding: "10px 16px",
      borderRadius: 10,
      fontSize: 13,
      fontWeight: 700,
      background: reviewAiLoading || !reviewAiInput.trim() ? C.dim : `linear-gradient(135deg,${C.accent},${C.accentLight})`,
      color: reviewAiLoading || !reviewAiInput.trim() ? C.muted : "#fff",
      border: "none",
      cursor: reviewAiLoading || !reviewAiInput.trim() ? "not-allowed" : "pointer",
      flexShrink: 0
    }
  }, "→")))) : null;
  ...
  if (!authUser) {
    ...
    return wrap(/*#__PURE__*/React.createElement("div", {
      ...
      style: {
        ...
        fontSize: 14,
        background: C.surface,
        border: `1.5px solid ${authPassword.length >= 6 ? C.accent : C.border}`,
        color: C.text,
        outline: "none",
        marginBottom: 10,
        boxSizing: "border-box"
  ...
  if (screen === "home") return wrap(/*#__PURE__*/React.createElement(React.Fragment, null, settingsOpen && /*#__PURE__*/React.createElement("div", {
  ...
  }, "🎓 CFA Level"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 6
    }
    ...
    onClick: () => {
      setCfaLevel(l);
      try {
        localStorage.setItem(CFA_LEVEL_KEY, l);
      } catch {}
    },
    style: {
      flex: 1,
  ...
  }, "Affects AI tutor, question generation, and study notes")), /*#__PURE__*/React.createElement("div", {
    style: {
      width: "100%",
      display: "flex",
      alignItems: "center",
  ...
  }, "AI Features"), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      color: C.muted,
      marginTop: 1
    }
  }, "Powered by ClearCFA backend — no key needed")), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
      color: C.easy,
      fontWeight: 700
  ...
  }, supabaseSyncing ? "Syncing…" : `${history.length} sessions · ${Object.keys(srDeckRef.current).length} SR cards${driveStatus === "synced" ? " · synced ✓" : driveStatus === "error" ? " · sync failed ✗" : " · tap to sync"}`)), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 11,
  ...
  }, "CFA Level ", cfaLevel, " Prep · ", /*#__PURE__*/React.createElement("span", {
    style: {
      color: C.accent,
      cursor: "pointer"
    },
  ...
  }, weeklyPlan ? "Plan · AI gaps · SR · leeches" : "AI-powered · LOS gaps · SR · leeches")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 6,
      alignItems: "center"
    }
  }, weeklyPlan && /*#__PURE__*/React.createElement("button", {
  ...
  }, "Full Plan"), !focusLoading && /*#__PURE__*/React.createElement("button", {
    onClick: () => {
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "📅 From Your Weekly Plan"), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: startedCount === todayPlanSessions.length ? C.easy : C.muted,
        fontWeight: startedCount === todayPlanSessions.length ? 700 : 400
      }
    }, startedCount === todayPlanSessions.length ? "✓ All done!" : startedCount + "/" + todayPlanSessions.length + " done")), /*#__PURE__*/React.createElement("div", {
      style: {
        height: 4,
        borderRadius: 2,
        background: C.border,
      ...
      style: {
        ...
        borderRadius: 2,
        background: `linear-gradient(90deg,${C.accent},${C.accentLight})`,
        width: `${startedCount / todayPlanSessions.length * 100}%`,
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        style: {
          ...
          alignItems: "center",
          padding: "11px 12px",
          background: done ? "rgba(34,197,94,0.05)" : C.surfaceHigh,
      ...
      }, session.module, " · ", session.durationMin, "min · ", session.count, "Q · ", session.difficulty), !done && /*#__PURE__*/React.createElement("div", {
        style: {
          fontSize: 10,
          color: C.muted,
          fontStyle: "italic",
        ...
        onClick: () => {
          ...
          try {
            localStorage.setItem("cfa_today_started", JSON.stringify(newStarted));
          } catch {}
          if (session.type === "sr") {
        ...
        style: {
          ...
          padding: "7px 12px",
          borderRadius: 8,
          background: done ? C.surface : `linear-gradient(135deg,${C.accent},${C.accentLight})`,
          color: done ? C.muted : "#fff",
  ...
  })(), focusLoading && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: 9
  ...
  }, "Claude analyses your accuracy trends, SR due cards, leech cards, and LOS coverage gaps to recommend what to drill today.")), focusSuggestions && !focusLoading && (() => {
    const todayName = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"][new Date().getDay()];
    const todayPlanSessions = (weeklyPlan?.days || []).find(d => d.day === todayName)?.sessions || [];
    ...
    return /*#__PURE__*/React.createElement("div", null, todayPlanSessions.length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 10,
        fontWeight: 800,
        color: C.muted,
    ...
    }, "Also suggested by AI"), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        flexDirection: "column",
        gap: 8
      }
    }, deduped.map((s, i) => /*#__PURE__*/React.createElement("div", {
      key: i,
  ...
  if (screen === "srReview") {
    ...
    }, "💬 Ask AI")), card.explanation), card.los_tested && /*#__PURE__*/React.createElement("div", {
      style: {
        background: C.dim,
        borderRadius: 8,
        padding: "8px 12px",
    ...
    }, srIdx < srQueue.length - 1 ? "Next Card →" : "Finish Review ✓")), /*#__PURE__*/React.createElement(ReviewAIChatPanel, null)));
  }
  ...
  if (screen === "setup") return wrap(/*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
    ...
    const adaptiveDiff = (() => {
      ...
      mSessions.forEach(h => {
        ...
        byDiff[h.difficulty].total += h.pct || 0;
  ...
  if (screen === "quiz") {
    ...
    return wrap(/*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      ...
      onClick: async () => {
        if (!authUser?.userId) {
          setExplainThisText("Sign in to use Explain This.");
          return;
        }
  ...
  if (screen === "results") {
    ...
    }, "📋 ", wrongs.length, " wrong answer", wrongs.length !== 1 ? "s" : "", " added to SR deck with LOS tags + misconception flags."), authUser?.userId && wrongs.length > 0 && /*#__PURE__*/React.createElement("div", {
      style: {
        background: "#080818",
        border: `1px solid #22d3ee33`,
        borderRadius: 12,
...
Give a 3-sentence debrief: (1) root cause of errors, (2) one specific thing to do next, (3) one honest motivational sentence. Be direct and specific, not generic. No markdown.`;
  ...
  if (screen === "readiness") return wrap(/*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    ...
    }, history.length < 10 && passProbability.label === "At Risk" ? "Early estimate — sharpen after 10+ sessions." : passProbability.advice), /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 12,
        color: C.muted,
        lineHeight: 1.5
      }
    }, predicted ? `${predicted.modulesWithData}/10 topics with reliable data. Weighted by CFA official exam weights.` : passProbability ? "" : "Complete ≥10 questions across 3+ topics to unlock a weighted estimate."), predicted && /*#__PURE__*/React.createElement("div", {
      style: {
        fontSize: 11,
        color: C.muted,
        marginTop: 5
  ...
  })(), passTrend.length >= 2 && passProbability && /*#__PURE__*/React.createElement("div", {
    style: {
      background: C.surface,
      border: `1px solid ${C.border}`,
      borderRadius: 12,
    ...
    const pts = passTrend.map((p, i) => ({
      x: n === 1 ? 180 : Math.round(i / (n - 1) * 340 + 10),
      y: Math.round(80 - p.prob / 100 * 70),
      p
    }));
  ...
  }, "today"))), passProbability && /*#__PURE__*/React.createElement("div", {
    style: {
      background: C.surface,
      border: `1px solid ${C.border}`,
      borderRadius: 12,
  ...
  }))))), passProbability && /*#__PURE__*/React.createElement("div", {
    style: {
      background: C.surface,
      border: `1px solid ${C.border}`,
      borderRadius: 12,
    ...
    }, m.accuracy != null ? `${m.accuracy}% · ` : "", "last drilled ", m.lastDate || "a while ago")), /*#__PURE__*/React.createElement("button", {
      onClick: () => {
        setScreen("home");
  ...
  }, "⚠ Directional estimate only — based on AI-generated questions, not validated CFA Institute content. Use it to guide focus, not as a definitive prediction.")), 620);
  ...
  if (screen === "dashboard") {
    ...
    history.forEach(s => {
      ...
      subMap[k].correct += s.score || 0;
      subMap[k].total += s.total || 0;
      subMap[k].sessions += 1;
    });
  ...
  if (screen === "review") {
    ...
    }, "💬 Ask AI")), w.explanation), w.los_tested && /*#__PURE__*/React.createElement("div", {
      style: {
        background: C.dim,
        borderRadius: 8,
        padding: "8px 12px",
    ...
    }, "Done ✓")), /*#__PURE__*/React.createElement(ReviewAIChatPanel, null)));
  }
  // ══ API KEY SCREEN ══════════════════════════════════════════════════════════
  if (screen === "apiKey") {
    setScreen("home");
  ...
  if (screen === "calcTrainer") return wrap((() => {
    ...
    const generateCalcProblem = async () => {
      if (!authUser?.userId) {
        setCalcError("Sign in to use Calc Trainer.");
        return;
      }
      setCalcLoading(true);
      setCalcError("");
  ...
  if (screen === "walkthrough") return wrap((() => {
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      ...
      onClick: async () => {
        if (!authUser?.userId) {
          setWalkthroughError("Sign in to use Concept Walkthrough.");
          return;
        }
