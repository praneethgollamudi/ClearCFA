...
function buildQuestionPrompt(topic,module,difficulty,count,level="1",losData=null,miscData=null,dynCtx=null,multiModuleList=null,seenStems=[],testedLOS=[],style="standard"){
  ...
  const levelGuidance=level==="2"
    ...
    :level==="3"
    ?`CFA Level 3 format: Frame every question around a PORTFOLIO MANAGEMENT decision — client objectives, IPS constraints, asset allocation, or risk management. Questions should require the candidate to recommend or justify, not just recall. Complexity: ${difficulty==="Easy"?"given a client with one clearly stated constraint (e.g. liquidity need, time horizon, risk tolerance), identify the single most appropriate portfolio decision or IPS element. Distractors represent decisions that suit a different client type or violate the one stated constraint":difficulty==="Medium"?"recommend a specific course of action given 2–3 client constraints from the scenario, then justify with one concrete data point. One distractor must represent the opposite recommendation with a flawed but superficially plausible rationale; another must reflect a correct principle applied to the wrong client constraint":"multi-constraint portfolio construction or behavioural bias evaluation"}.`
    :`Difficulty: ${difficulty==="Easy"?"identify what a term, concept, or standard IS or what it is used for. The question gives a one-sentence definition or scenario and asks the student to name/identify the correct concept. Distractors must be real CFA terms from the same topic that are commonly confused with the correct answer — never made-up terms or obviously unrelated options. No calculation required":difficulty==="Medium"?"single formula applied to a realistic scenario with specific numbers — student must identify the right formula, extract the correct inputs, and compute the answer. Distractors must use the correct formula with a wrong input (e.g. wrong rate, wrong period, pre-tax instead of after-tax) OR a related but incorrect formula. Do NOT ask what a formula means — give a scenario and make the student use it":"multi-step chain requiring 2+ formula applications in sequence (e.g. compute a metric then use it in a valuation or comparison), OR a judgment question using 'EXCEPT'/'LEAST likely' that tests a subtle distinction between closely related concepts. NEVER ask what a formula is — ask what a result IMPLIES, which assumption changes the answer, or which scenario violates a rule. Wrong options must reflect plausible but flawed reasoning, not obvious errors"}`;
...
- "calc_steps" (REQUIRED in every question): If the question can be solved faster with a BA II Plus calculator (TVM bond pricing, annuity PV/FV, YTM solve, NPV/IRR, EAR/APR conversion via ICONV, loan payment/amortization), set applicable:true, worksheet to "TVM"|"CF"|"ICONV"|"Amort", and keys to the COMPLETE keystroke sequence using THIS question's specific numbers — e.g. for a 5-year 8% coupon bond at YTM 6%: ["[2ND]","[CLR TVM]","5","[N]","6","[I/Y]","80","[PMT]","1000","[FV]","[CPT]","[PV]"]. Set result to the signed numeric answer (e.g. "−1,084.25"). For CF worksheet with repeated identical cash flows, use the frequency field (e.g. ["[CF]","[2ND]","[CLR WORK]","-50000","[ENTER]","[↓]","15000","[ENTER]","[↓]","5","[ENTER]","[↓]","[NPV]","10","[ENTER]","[↓]","[CPT]"]) — NEVER use "…" or ellipsis anywhere in the keys array; write out every single keystroke. For conceptual/qualitative questions with no numeric shortcut, set applicable:false and omit worksheet/keys/result.
...
- SELF-CONSISTENCY RULE for comparative preference questions: If your question asks "when should [X] be preferred over [Y]" or "[X] is preferred if and only if [condition]" or "an investor should choose [X] when", the correct answer MUST be a condition that genuinely makes [X] optimal, and the explanation MUST conclude [X] is the right choice. NEVER write a question asking when [X] is preferred if [X] is always dominated by [Y] — that premise is impossible and produces an invalid question. Fatal error example: question says "prefer Alpha" but explanation concludes "Beta is always preferred" — this will be discarded.`;
}
