...
function buildQuestionPrompt(topic,module,difficulty,count,level="1",losData=null,miscData=null,dynCtx=null,multiModuleList=null,seenStems=[],testedLOS=[]){
  ...
  const levelGuidance=level==="2"
    ...
    :`Difficulty: ${difficulty==="Easy"?"recall/definition":difficulty==="Medium"?"apply formula to scenario with numbers":"multi-step chain requiring 2+ formula applications in sequence (e.g. compute a metric then use it in a valuation or comparison), OR a judgment question using 'EXCEPT'/'LEAST likely' that tests a subtle distinction between closely related concepts. NEVER ask what a formula is — ask what a result IMPLIES, which assumption changes the answer, or which scenario violates a rule. Wrong options must reflect plausible but flawed reasoning, not obvious errors"}`;
