...
const FORMULAS = {
  "Quantitative Methods": [
    ...
    {name:"Sample Variance",f:"s² = Σ(xᵢ − x̄)² / (n−1)"},
    {name:"Covariance",f:"Cov(A,B) = ρ × σ_A × σ_B"},
    {name:"Correlation",f:"ρ = Cov(A,B) / (σ_A × σ_B)"},
    {name:"Portfolio σ (2 asset)",f:"σ_p = √(w²_Aσ²_A + w²_Bσ²_B + 2w_Aw_BρABσ_Aσ_B)"},
    {name:"t-statistic",f:"t = (x̄ − μ₀) / (s / √n)"},
    {name:"Confidence interval",f:"x̄ ± z_{α/2} × (σ / √n)"},
    {name:"Regression slope",f:"b = Cov(X,Y) / Var(X)"},
    {name:"R² (coefficient of det.)",f:"R² = 1 − SSE/SST"},
    {name:"F-stat (regression)",f:"F = MSR / MSE = (R²/k) / ((1−R²)/(n−k−1))"},
    {name:"Chi-square (variance test)",f:"χ² = (n−1)s² / σ₀²"},
    {name:"Safety-first ratio",f:"SFR = (E(R) − R_min) / σ"},
    {name:"Bayes",f:"P(A|B) = P(B|A)·P(A) / P(B)"},
    {name:"Combination",f:"ⁿCᵣ = n! / (r!(n−r)!)"},
    {name:"Permutation",f:"ⁿPᵣ = n! / (n−r)!"},
  ],
  "Economics": [
    ...
    {name:"Fisher equation",f:"(1 + r_nominal) = (1 + r_real)(1 + inflation)  ≈  r_nominal ≈ r_real + π"},
    {name:"Fiscal multiplier",f:"Multiplier = 1 / (1 − MPC)"},
    {name:"Tax multiplier",f:"Tax multiplier = −MPC / (1 − MPC)"},
    {name:"Absolute PPP",f:"S = P_domestic / P_foreign"},
    {name:"Relative PPP",f:"%ΔS ≈ π_domestic − π_foreign"},
    {name:"Covered interest parity",f:"F/S = (1 + r_d) / (1 + r_f)"},
    {name:"Uncovered interest parity",f:"E(%ΔS) = r_domestic − r_foreign"},
    {name:"Real exchange rate",f:"q = S × (P_foreign / P_domestic)"},
    {name:"Current account balance",f:"CA = X − M + Net income + Net transfers"},
    {name:"Elasticity of demand",f:"E_d = %ΔQ_d / %ΔP"},
  ],
  "Financial Statement Analysis": [
    ...
    {name:"Cash Ratio",f:"(Cash + ST Investments) / CL"},
    {name:"DSO (days sales outstanding)",f:"AR / (Revenue / 365)"},
    {name:"DIO (days inventory outstanding)",f:"Inventory / (COGS / 365)"},
    {name:"DPO (days payable outstanding)",f:"AP / (COGS / 365)"},
    {name:"CCC (cash conversion cycle)",f:"CCC = DSO + DIO − DPO"},
    {name:"Basic EPS",f:"(NI − Preferred Dividends) / Wtd Avg Shares"},
    {name:"Diluted EPS",f:"(NI − Pref Div + Convertible Interest(1−t)) / (Wtd Avg + Dilutive Shares)"},
    {name:"Debt-to-Equity",f:"Total Debt / Total Equity"},
    {name:"Debt-to-Assets",f:"Total Debt / Total Assets"},
    {name:"Interest Coverage",f:"EBIT / Interest Expense"},
    {name:"FCFF",f:"FCFF = NI + NCC + Int(1−t) − FCInv − WCInv"},
    {name:"FCFE",f:"FCFE = FCFF − Int(1−t) + Net Borrowing  OR  NI + NCC − FCInv − WCInv + Net Borrowing"},
    {name:"CFO (indirect)",f:"CFO = NI + D&A − ΔAR − ΔInventory + ΔAP ± other working capital"},
  ...
  "Corporate Issuers": [
    ...
    {name:"Profitability Index",f:"PI = PV(future CFs) / Initial Cost  (accept if PI > 1)"},
    ...
    {name:"Cost of equity (DDM)",f:"r_e = D₁/P₀ + g"},
    {name:"Cost of debt",f:"r_d(after-tax) = YTM × (1 − Tax Rate)"},
    {name:"DOL (degree of operating lev.)",f:"DOL = % ΔOP Income / % ΔRevenue  =  (Rev − VarCosts) / EBIT"},
    {name:"DFL (degree of financial lev.)",f:"DFL = % ΔEPS / % ΔOP Income  =  EBIT / (EBIT − Interest)"},
    {name:"DTL (total leverage)",f:"DTL = DOL × DFL"},
    {name:"Breakeven (units)",f:"Q_BE = Fixed Costs / (Price − Variable Cost per unit)"},
    {name:"M-M (with taxes)",f:"V_L = V_U + T×D"},
  ],
  "Equity": [
    {name:"Gordon Growth Model (DDM)",f:"V₀ = D₁ / (r − g)"},
    {name:"Two-stage DDM",f:"V₀ = Σ Dₜ/(1+r)ᵗ  +  P_n/(1+r)ⁿ  where P_n = D_{n+1}/(r−g)"},
    {name:"FCFE",f:"FCFE = NI − (1−DR)(FCInv − Dep) − (1−DR)ΔWC"},
    {name:"P/E (justified leading)",f:"P/E = Payout Ratio / (r − g)"},
    {name:"P/B (justified)",f:"P/B = (ROE − g) / (r − g)"},
    {name:"EV/EBITDA",f:"EV = Mkt Cap + Debt − Cash;  EV/EBITDA compares across capital structures"},
    {name:"Enterprise Value",f:"EV = Mkt Cap + Total Debt + Pref − Cash & Equivalents"},
    {name:"CAPM",f:"E(R_i) = R_f + β_i[E(R_m) − R_f]"},
    {name:"Beta",f:"β = Cov(R_i, R_m) / Var(R_m)  =  ρ_{i,m} × σ_i / σ_m"},
    {name:"Sharpe Ratio",f:"S = (R_p − R_f) / σ_p"},
    {name:"Treynor Ratio",f:"T = (R_p − R_f) / β_p"},
    {name:"Jensen's Alpha",f:"α = R_p − [R_f + β_p(R_m − R_f)]"},
  ],
  "Fixed Income": [
    ...
    {name:"Convexity (approx.)",f:"Convexity ≈ (P₊ + P₋ − 2P₀) / (P₀ × (Δy)²)"},
...
[{"id":"q1","question":"…","options":{"A":"…","B":"…","C":"…","D":"…"},"answer":"A","explanation":"…","concept":"…","los_tested":"LOS X.X","misconception_targeted":"…","_topic":"<exact topic name from list above>","_subtopic":"<module name>"}]`;
  ...
  if(screen==="home") return wrap(<>
    ...
    <div style={{display:"flex",gap:8,marginBottom:8}}>
      ...
      <button onClick={()=>{trackUsage("ai_coach");setAiCoachScreen(true);}} style={{flex:1,padding:"11px",borderRadius:11,fontSize:12,fontWeight:600,background:"#22d3ee18",border:`1px solid #22d3ee44`,color:"#22d3ee",cursor:"pointer"}}>🤖 Coach</button>
    </div>
    {/* Lucky Dip */}
    <button onClick={()=>{
    ...
    {(()=>{
      const moreItems=[
        ...
        {key:"ethics",label:"⚖️ Ethics",style:{background:C.hard+"12",border:`1px solid ${C.hard}44`,color:C.hard},action:()=>{trackUsage("ethics");const cases=getEthicsCases("all",10);if(cases.length){setTopic("Ethics");setSubtopic("Ethics Case Studies");setDifficulty("Medium");setCount(cases.length);setMode("guided");setQuestions(cases);setAnswers({});setCurrentQ(0);setShowExp(false);setLastSession(null);setFullExamMode(false);setVignetteMode(false);setScreen("quiz");}}},
