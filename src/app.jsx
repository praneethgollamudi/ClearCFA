...
function getSupabaseConfig(){
  try{return JSON.parse(localStorage.getItem("cfa_supabase_config")||"null");}catch{return null;}
}
...
async function supabaseSync(cfg, history, srDeck){
  ...
  try{
    ...
    await fetch(url,{method,headers:{"apikey":cfg.key,"Authorization":`Bearer ${cfg.key}`,"Content-Type":"application/json","Prefer":"return=minimal"},body:JSON.stringify(payload)});
    return true;
  }catch{return false;}
}
...
function getModuleReadiness(history){
  const now=Date.now();
  return Object.entries(LOS).map(([topic,{weight,modules}])=>{
    ...
    return{topic,weight,modules:moduleNames,modulesCovered,untouchedModules,moduleStats,losStats,sessions:sessions.length,totalQs,accuracy,coverage,readiness,reliable,trend,trendDelta,lastSession:sessions.length?new Date(sessions[0].id).toLocaleDateString("en-IN",{day:"numeric",month:"short"}):null};
  });
}
...
const FORMULAS = {
  "Quantitative Methods": [
    ...
    {name:"Variance",f:"σ² = Σ(xᵢ − μ)² / N"},
    {name:"Covariance",f:"Cov(A,B) = ρ × σ_A × σ_B"},
    {name:"Portfolio σ",f:"σ_p = √(w²_Aσ²_A + w²_Bσ²_B + 2w_Aw_BρABσ_Aσ_B)"},
    {name:"Safety-first",f:"SFR = (E(R) − R_min) / σ"},
    {name:"Bayes",f:"P(A|B) = P(B|A)·P(A) / P(B)"},
  ],
  "Financial Statement Analysis": [
    {name:"ROE (DuPont)",f:"ROE = Net Margin × Asset Turnover × Equity Multiplier"},
    {name:"ROE (3-factor)",f:"ROE = (NI/Sales) × (Sales/Assets) × (Assets/Equity)"},
    {name:"Current Ratio",f:"Current Assets / Current Liabilities"},
    {name:"Quick Ratio",f:"(Cash + ST Investments + AR) / CL"},
    {name:"DSO",f:"365 / (Revenue / AR)"},
    {name:"DIO",f:"365 / (COGS / Inventory)"},
    {name:"DPO",f:"365 / (COGS / AP)"},
    {name:"CCC",f:"DSO + DIO − DPO"},
    {name:"Basic EPS",f:"(NI − Pref Div) / Wtd Avg Shares"},
    {name:"Diluted EPS",f:"(NI − Pref Div + Convertible Int) / (Wtd Avg + Dilutive Shares)"},
    {name:"Debt-to-Equity",f:"Total Debt / Total Equity"},
    {name:"Interest Coverage",f:"EBIT / Interest Expense"},
  ],
  "Fixed Income": [
    {name:"Bond Price",f:"P = Σ C/(1+y)ᵗ + FV/(1+y)ⁿ"},
    {name:"Current Yield",f:"CY = Annual Coupon / Price"},
    {name:"Macaulay Duration",f:"D = Σ[t × PV(CFₜ)] / Price"},
    {name:"Modified Duration",f:"MD = Macaulay D / (1 + y/m)"},
    {name:"Price Change (Duration)",f:"ΔP/P ≈ −MD × Δy"},
    {name:"Full Price w/ Convexity",f:"ΔP/P ≈ −MD·Δy + ½·Convexity·(Δy)²"},
    {name:"PVBP",f:"PVBP = MD × Price × 0.0001"},
    {name:"Yield Spread",f:"Spread = YTM_bond − YTM_benchmark"},
    {name:"Forward Rate",f:"(1+S₂)² = (1+S₁)(1+₁f₁)"},
  ],
  "Equity": [
    {name:"Gordon Growth (DDM)",f:"V = D₁ / (r − g)"},
    {name:"Two-stage DDM",f:"V = PV(Divs stage 1) + PV(Terminal Value)"},
    {name:"FCFE",f:"FCFE = NI + Dep − CapEx − ΔNWC + Net Borrowing"},
    {name:"P/E (leading)",f:"P/E = Div payout / (r − g)"},
    {name:"EV",f:"EV = Mkt Cap + Debt − Cash"},
    {name:"CAPM",f:"E(R) = Rf + β(E(Rm) − Rf)"},
    {name:"Beta",f:"β = Cov(R_i, R_m) / Var(R_m)"},
    {name:"Sharpe",f:"S = (Rp − Rf) / σ_p"},
    {name:"Treynor",f:"T = (Rp − Rf) / β"},
    {name:"Jensen's α",f:"α = Rp − [Rf + β(Rm − Rf)]"},
  ],
  "Derivatives": [
    {name:"Call Payoff (long)",f:"max(S_T − X, 0)"},
    {name:"Put Payoff (long)",f:"max(X − S_T, 0)"},
    {name:"Put-Call Parity",f:"C + PV(X) = P + S"},
    {name:"Forward Price",f:"F = S₀ × (1 + r)ᵀ"},
    {name:"FRA Settlement",f:"(L − FRA_rate) × D/360 × NP / (1 + L × D/360)"},
    {name:"Swap (fixed rate)",f:"Fixed rate where PV(fixed) = PV(floating)"},
  ],
  "Corporate Issuers": [
    {name:"WACC",f:"WACC = w_d×r_d(1−t) + w_e×r_e"},
    {name:"NPV",f:"NPV = Σ CF_t/(1+r)ᵗ − Initial Cost"},
    {name:"IRR",f:"NPV = 0 → solve for r"},
    {name:"Operating Leverage",f:"DOL = % ΔOP Income / % ΔRevenue"},
    {name:"Financial Leverage",f:"DFL = % ΔEPS / % ΔOP Income"},
    {name:"D/E Modigliani-Miller",f:"V_L = V_U + T×D (with taxes)"},
  ],
  "Alternatives": [
    {name:"NAV per share",f:"NAV = (Assets − Liabilities) / Shares Outstanding"},
    {name:"PE Return",f:"IRR: solve PV(invested) = PV(exit proceeds)"},
    {name:"Mgmt Fee (PE)",f:"Typically 2% × Committed Capital"},
    {name:"Carried Interest",f:"Typically 20% × Profits above hurdle rate"},
    {name:"Cap Rate (RE)",f:"Cap Rate = NOI / Property Value"},
    {name:"Gross Return",f:"R_gross = (NAV_end + Distributions) / NAV_beg − 1"},
  ],
};
        ...
        <div style={{background:"#08081a",border:`1px solid ${C.accentLight}22`,borderRadius:"0 0 10px 10px",padding:"10px 14px",display:"flex",flexDirection:"column",gap:6}}>
          {formulas.map((f,i)=>(
            <div key={i} style={{display:"flex",gap:10,alignItems:"flex-start",paddingBottom:6,borderBottom:i<formulas.length-1?`1px solid ${C.border}`:"none"}}>
              ...
              <div style={{fontSize:12,color:C.accentLight,fontFamily:"monospace",lineHeight:1.5}}>{f.f}</div>
            </div>
          ))}
        </div>
      )}
            ...
            <>
              {formulaData.length===0?(
                ...
                <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,overflow:"hidden"}}>
                  {formulaData.map((f,i)=>(
                    <div key={i} style={{display:"flex",gap:12,alignItems:"flex-start",padding:"11px 16px",borderBottom:i<formulaData.length-1?`1px solid ${C.border}`:"none"}}>
...
"Ethics": [
  ...
  ()=>{
    const name=pname();const gift=rnd(150,500);const threshold=100;
    return{question:`${name}, CFA, is offered a gift worth $${gift} from a client whose portfolio she manages, as a thank-you for strong performance. She has NOT yet notified her employer. What should she do?`,options:{A:`Accept the gift since it is a gesture of appreciation, not a bribe`,B:`Decline the gift as it exceeds the $${threshold} threshold and she has not obtained employer permission`,C:`Accept the gift but disclose it to her employer within 30 days`},answer:"B",explanation:`Standard I-B requires written permission from employer before accepting gifts above $${threshold} from clients. Accepting first and disclosing later is not compliant. She must disclose and get approval BEFORE accepting.`,concept:"Standard I-B",los_tested:"demonstrate application of Standard I-B: Independence and Objectivity",misconception_targeted:"thinking disclosure after the fact is sufficient"};
  },
  ...
  ()=>{
    ...
    return{question:`${name}, CFA, ${info} suggesting the company's stock will rise significantly. She buys shares for client accounts before the information becomes public. Which Standard is MOST likely violated?`,options:{A:`Standard I-C: Misrepresentation`,B:`Standard II-A: Material Nonpublic Information`,C:`Standard III-A: Loyalty, Prudence and Care`},answer:"B",explanation:`Standard II-A prohibits trading on material nonpublic information. Information is material if it would affect a reasonable investor's decision. Acting on this before public release violates this Standard.`,concept:"Standard II-A",los_tested:"demonstrate application of Standard II-A: Material Nonpublic Information",misconception_targeted:"confusing duties to clients with market integrity standards"};
  },
  ...
  ()=>{ const cases=getEthicsCases("all",1); return cases.length?cases[0]:null; },
  ()=>{ const cases=getEthicsCases("all",1); return cases.length?cases[0]:null; },
...
"Fixed Income": [
  ...
  ()=>{
    const md=parseFloat(rnd(40,90)/10).toFixed(1);const dy=parseFloat(rnd(20,80)/100).toFixed(2);const price=rnd(95,105);
    ...
    return{question:`A bond has a modified duration of ${md} and is priced at $${price}. If yields rise by ${dy}%, what is the approximate price change?`,options:{A:`$${dp}`,B:`$${wrong1} (positive — price rises with yields)`,C:`$${wrong2} (incorrect scaling)`},answer:"A",explanation:`ΔP ≈ −ModDuration × Δy × Price = −${md} × ${parseFloat(dy)/100} × $${price} = $${dp}. The negative sign reflects the inverse price-yield relationship. A ${dy}% yield rise causes approximately a $${Math.abs(parseFloat(dp))} price decline.`,concept:"Modified Duration",los_tested:"define calculate and interpret modified duration money duration and the price value of a basis point",misconception_targeted:"forgetting the negative sign in the duration price change formula"};
  },
  ...
  ()=>{
    ...
    return{question:`A bond has a Macaulay duration of ${mac} years and a yield-to-maturity of ${y}% compounded ${m===1?"annually":"semi-annually"}. What is its modified duration?`,options:{A:`${mod} years`,B:`${wrong1} years (same as Macaulay)`,C:`${wrong2} years`},answer:"A",explanation:`Modified Duration = Macaulay Duration / (1 + y/m) = ${mac} / (1 + ${y}%/${m}) = ${mod}. Modified duration always < Macaulay duration. It directly measures the % price change for a 1% change in yield.`,concept:"Modified Duration",los_tested:"define calculate and interpret Macaulay duration",misconception_targeted:"confusing Macaulay and modified duration or treating them as equal"};
  },
  // Coupon and duration relationship
  ()=>{
    const c1=rnd(2,5),c2=rnd(8,12);const mat=rnd(5,15);
    return{question:`Two bonds have identical maturities of ${mat} years and the same yield-to-maturity. Bond A has a coupon of ${c1}% and Bond B has a coupon of ${c2}%. Which bond has higher interest rate risk?`,options:{A:`Bond A (${c1}% coupon) — lower coupon means higher duration`,B:`Bond B (${c2}% coupon) — higher coupon means more cash flows to be discounted`,C:`Both bonds have identical interest rate risk since maturity and yield are the same`},answer:"A",explanation:`Lower coupon → higher duration → higher price sensitivity to yield changes. Bond A (${c1}% coupon) returns less cash earlier, so the weighted average time to receive cash flows (duration) is longer. Bond B's higher coupons reduce its duration despite identical maturity.`,concept:"Coupon and Duration",los_tested:"explain how a bond's maturity coupon and yield level affect its interest rate risk",misconception_targeted:"ignoring coupon rate's effect on duration when maturity is equal"};
  },
],
...
"Equity": [
  ...
  ()=>{
    ...
    return{question:`A stock just paid a dividend of $${d0} per share. The dividend is expected to grow at ${g}% per year indefinitely. The required return is ${r}%. What is the intrinsic value per share?`,options:{A:`$${v}`,B:`$${wrong1} (uses D₀ instead of D₁)`,C:`$${wrong2} (adds g instead of subtracting)`},answer:"A",explanation:`Gordon Growth Model: V = D₁/(r−g). D₁ = D₀×(1+g) = $${d0}×(1+${g}%) = $${d1}. V = $${d1}/(${r}%−${g}%) = $${v}. Always use next period's dividend (D₁), not the just-paid dividend (D₀).`,concept:"Gordon Growth Model",los_tested:"calculate and interpret the intrinsic value of an equity security based on the Gordon growth dividend discount model",misconception_targeted:"using D₀ instead of D₁ in the Gordon Growth Model"};
  },
  // P/E and growth
  ()=>{
    ...
    return{question:`A company has a dividend payout ratio of ${pout}%, required return of ${r}%, and sustainable growth rate of ${g}%. What is the justified leading P/E?`,options:{A:`${pe}×`,B:`${wrong1}× (ignores payout ratio)`,C:`${wrong2}× (uses retention ratio instead of payout)`},answer:"A",explanation:`Justified leading P/E = Payout ratio / (r − g) = ${pout}% / (${r}% − ${g}%) = ${pe}×. The payout ratio reflects what fraction of earnings is distributed. Higher payout or lower (r−g) spread = higher justified P/E.`,concept:"P/E Valuation",los_tested:"explain the rationale for using price multiples to value equity and how the price to earnings multiple relates to fundamentals",misconception_targeted:"using retention ratio instead of payout ratio in the P/E formula"};
  },
  // Market Efficiency
  ()=>{
    ...
    return{question:`If markets are ${form}-form efficient, which statement BEST describes the implication for investment analysis?`,options:{A:wrongA,B:implication,C:"Passive management always outperforms active management regardless of efficiency form"},answer:"B",explanation:`${form.charAt(0).toUpperCase()+form.slice(1)}-form efficiency: ${implication} Each level of efficiency subsumes the previous — semi-strong includes weak-form, strong includes both.`,concept:"Market Efficiency",los_tested:"contrast weak-form semi-strong-form and strong-form market efficiency",misconception_targeted:"confusing which forms of analysis are rendered ineffective by each EMH form"};
  },
],
...
"Corporate Issuers": [
  ...
  ()=>{
    ...
    return{question:`A firm has ${wd}% debt (cost ${rd}%) and ${we}% equity (cost ${re}%), with a tax rate of ${t}%. What is the WACC?`,options:{A:`${wacc}%`,B:`${wrong1}% (no tax shield on debt)`,C:`${wrong2}% (simple average)`},answer:"A",explanation:`WACC = w_d×r_d×(1−t) + w_e×r_e = ${wd}%×${rd}%×(1−${t}%) + ${we}%×${re}% = ${wacc}%. The after-tax cost of debt reflects the interest tax shield. Use market value weights, not book value.`,concept:"WACC",los_tested:"calculate and interpret the weighted-average cost of capital for a company",misconception_targeted:"ignoring the tax shield on debt when calculating WACC"};
  },
  // NPV vs IRR conflict
  ()=>{
    const npvA=rnd(50,200);const npvB=rnd(30,160);const irrA=rnd(15,22);const irrB=rnd(18,28);
    const winner=npvA>npvB?"A":"B";const irrwinner=irrA>irrB?"A":"B";
    if(irrwinner===winner) {irrA+=5;}// ensure conflict
    return{question:`Project A has NPV = $${npvA}K and IRR = ${irrA}%. Project B has NPV = $${npvB}K and IRR = ${irrB}%. The projects are mutually exclusive. Which should be selected?`,options:{A:`Project ${winner} — NPV is the correct decision criterion for mutually exclusive projects`,B:`Project ${irrA>irrB?"A":"B"} — always choose the higher IRR`,C:`Cannot determine without knowing the cost of capital`},answer:"A",explanation:`When NPV and IRR conflict for mutually exclusive projects, use NPV. NPV measures absolute value creation, which is the firm's goal. IRR assumes reinvestment at the IRR rate — often unrealistic. Project ${winner} (NPV = $${Math.max(npvA,npvB)}K) creates more value.`,concept:"NPV vs IRR",los_tested:"describe the capital allocation process calculate NPV IRR and ROIC and contrast their use in capital allocation",misconception_targeted:"using IRR over NPV when they conflict for mutually exclusive projects"};
  },
],
...
function CFAMock(){
  ...
  useEffect(()=>{
    ...
    qs.forEach(q=>{
      ...
      const key=`${t}|||${st}|||${q.id}`;
      setSrDeck(prev=>{
        const existing=prev[key]||{concept:(q.concept||st).slice(0,60),topic:t,subtopic:st,question:(q.question||"").slice(0,180),options:q.options,answer:q.answer,explanation:(q.explanation||"").slice(0,300),los_tested:(q.los_tested||"").slice(0,100),wrongCount:0};
        ...
        if(isFlagged&&correct){
          ...
          updated.interval=1;updated.repetitions=0;
...
Reply with just "saved" when done.`}]
  ...
  const generateQuestions=async(t,st,diff,cnt,m="guided",isVignette=false)=>{
    ...
    if(!isVignette){
      ...
      if(localQs.length>=Math.min(cnt,3)){
        setLoadingProgress(100);setLoadingMsg(`${localQs.length} questions ready!`);
  ...
  const startFullExam=async(sessionNum=1)=>{
    ...
    try{
      const allTopics=Object.entries(LOS);const totalW=allTopics.reduce((s,[,{weight}])=>s+weight,0);
      let allQs=[];
  ...
  if(screen==="passProbability") return wrap(<>
    ...
    {!passProbability ? (
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px",marginBottom:14}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {moduleReadiness.filter(m=>m.sessions===0&&m.weight>=9).slice(0,3).map(m=>(
            <div key={m.topic} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 12px",background:C.dim,borderRadius:9}}>
              ...
              <button onClick={()=>{setScreen("home");setTimeout(()=>generateQuestions(m.topic,m.modules[0],"Easy",5,"guided"),100);}} style={{fontSize:11,fontWeight:700,padding:"5px 11px",borderRadius:7,background:C.accent+"22",border:`1px solid ${C.accent}44`,color:C.accentLight,cursor:"pointer"}}>5 Qs →</button>
            </div>
          ))}
          {moduleReadiness.filter(m=>m.accuracy!==null&&m.accuracy<65&&m.weight>=8).slice(0,2).map(m=>(
            <div key={m.topic} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 12px",background:C.dim,borderRadius:9}}>
              <div>
                <div style={{fontSize:13,fontWeight:600,color:C.text}}>{m.topic}</div>
                <div style={{fontSize:11,color:C.hard}}>{m.accuracy}% accuracy · needs work</div>
              </div>
              <button onClick={()=>{setScreen("home");setTimeout(()=>generateQuestions(m.topic,m.untouchedModules[0]||m.modules[0],"Medium",5,"guided"),100);}} style={{fontSize:11,fontWeight:700,padding:"5px 11px",borderRadius:7,background:C.hard+"22",border:`1px solid ${C.hard}44`,color:C.hard,cursor:"pointer"}}>Drill →</button>
            </div>
          ))}
        </div>
      </div>
  ...
  if(screen==="srReview"){
    ...
    return wrap(<>
      ...
      <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"20px",marginBottom:14,fontSize:14,lineHeight:1.8}}>{card.question}</div>
      {!srAnswer?(
  ...
  if(screen==="setup") return wrap(<>
    ...
    <div style={{marginBottom:18}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
        {Object.entries(TOPIC_MAP).map(([t,{weight}])=>(
    ...
    {topic&&(
      <div style={{marginBottom:18}}>
        ...
        {subtopic&&LOS[topic]?.modules[subtopic]&&(
          <div style={{marginTop:12,background:"#0a0a18",border:`1px solid ${C.border}`,borderRadius:9,padding:"12px 14px"}}>
            <div style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>LOS for this module ({LOS[topic].modules[subtopic].length} statements)</div>
            {LOS[topic].modules[subtopic].map((l,i)=>(
  ...
  if(screen==="quiz"){
    ...
    return wrap(<>
      ...
      {exitConfirm&&(
        <div style={{background:"#12101a",border:`1px solid ${C.hard}44`,borderRadius:12,padding:"16px",marginBottom:14,animation:"fadeIn 0.15s ease"}}>
          <div style={{fontSize:13,fontWeight:700,marginBottom:6}}>Exit session?</div>
          <div style={{fontSize:12,color:C.muted,marginBottom:12}}>Progress will be lost. SR deck won't update for this session.</div>
          <div style={{display:"flex",gap:9}}>
            <button onClick={()=>setExitConfirm(false)} style={{flex:1,padding:"9px",borderRadius:8,fontSize:13,fontWeight:600,background:C.surface,border:`1px solid ${C.border}`,color:C.muted,cursor:"pointer"}}>Continue</button>
            <button onClick={()=>{clearInterval(timerRef.current);setExitConfirm(false);setScreen("home");setFocusSuggestions(null);}} style={{flex:1,padding:"9px",borderRadius:8,fontSize:13,fontWeight:700,background:"#400010",border:`1px solid ${C.hard}44`,color:C.hard,cursor:"pointer"}}>Exit</button>
          </div>
        </div>
      )}
...
Give a 3-sentence debrief: (1) root cause of errors, (2) one specific thing to do next, (3) one honest motivational sentence. Be direct and specific, not generic. No markdown.`;
  ...
  if(screen==="readiness") return wrap(<>
    ...
    <div style={{background:predicted?(predicted.score>=70?"#041a0e":"#1a0407"):C.surface,border:`1px solid ${predicted?(predicted.score>=70?"#22a05a44":C.hard+"44"):C.border}`,borderRadius:13,padding:"18px 20px",marginBottom:16,display:"flex",alignItems:"center",gap:18}}>
      <ScoreRing pct={predicted?.score||0} size={84}/>
      <div style={{flex:1}}>
        <div style={{fontSize:14,fontWeight:700,marginBottom:4}}>{predicted?(predicted.score>=70?"On track to pass ✓":"Below passing threshold"):"Need more sessions to predict"}</div>
        {predicted&&<div style={{fontSize:13,color:predicted.score>=70?C.easy:C.hard,fontWeight:600,marginBottom:4}}>Range: {predicted.low}–{predicted.high}% <span style={{fontSize:11,color:C.muted,fontWeight:400}}>({predicted.confidence}% confidence)</span></div>}
        <div style={{fontSize:12,color:C.muted,lineHeight:1.5}}>{predicted?`${predicted.modulesWithData}/10 topics with reliable data. Weighted by CFA official exam weights.`:"Complete ≥10 questions across 3+ topics to unlock."}</div>
    ...
    <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"14px 16px",marginBottom:16}}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:6}}>
        {Object.entries(LOS).map(([topic,{modules}])=>{
    ...
    <div style={{marginTop:20,paddingTop:16,borderTop:`1px solid ${C.border}`}}>
      ...
      <div style={{marginBottom:8}}>
        ...
        <input value={supabaseUrl} onChange={e=>setSupabaseUrl(e.target.value)} placeholder="https://xxxx.supabase.co"
          style={{width:"100%",padding:"10px 12px",borderRadius:9,fontSize:12,background:C.surface,border:`1.5px solid ${supabaseUrl?"#44aa8844":C.border}`,color:C.text,outline:"none",boxSizing:"border-box"}}/>
      </div>
      <div style={{marginBottom:10}}>
        <label style={{fontSize:10,fontWeight:700,color:C.muted,letterSpacing:"0.08em",textTransform:"uppercase",display:"block",marginBottom:5}}>Anon Public Key</label>
      ...
      <div style={{display:"flex",gap:9,marginBottom:16}}>
        ...
        }} style={{flex:2,padding:"11px",borderRadius:9,fontSize:13,fontWeight:700,
          background:supabaseUrl&&supabaseKey?`linear-gradient(135deg,#22c55e,#16a34a)`:"#1a2a1a",
          color:supabaseUrl&&supabaseKey?"#fff":"#4a6a4a",border:"none",cursor:"pointer"}}>
          {supabaseSyncing?"Testing…":"Save & Test Connection"}
        </button>
        {supabaseCfg&&<button onClick={()=>{localStorage.removeItem("cfa_supabase_config");setSupabaseCfg(null);setSupabaseUrl("");setSupabaseKey("");}}
          style={{flex:1,padding:"11px",borderRadius:9,fontSize:12,fontWeight:600,background:"#200010",border:`1px solid ${C.hard}44`,color:C.hard,cursor:"pointer"}}>
          Disconnect
        </button>}
      </div>
