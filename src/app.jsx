...
function CFAMock(){
  ...
  useEffect(()=>{
    ...
    if(pct>=75&&diff!=="Hard"&&topicHistory.length>=2)setAutoEscalation({topic:t,subtopic:st,from:diff,to:diff==="Easy"?"Medium":"Hard"});
...
[{"id":"q1","question":"…","options":{"A":"…","B":"…","C":"…","D":"…"},"answer":"A","explanation":"…","concept":"…","los_tested":"LOS X.X","misconception_targeted":"…","_topic":"<exact topic name from list above>","_subtopic":"<module name>"}]`;
  ...
  const adaptiveOmDifficulty=useMemo(()=>{
    ...
    return avg>=75?"Hard":avg>=50?"Medium":"Easy";
  },[history]);
      ...
      <div style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:4,scrollbarWidth:"none",msOverflowStyle:"none"}}>
        {(()=>{
          ...
          return sortedTopics.map(({t,weight,mr},idx)=>{
            ...
            const diff=modPct===null?"Medium":modPct>=75?"Hard":modPct<45?"Easy":"Medium";
            return(
          ...
          {adminEngageResult&&(
            <div style={{background:adminEngageResult.error?C.hard+"12":C.easy+"12",borderRadius:9,padding:"10px 12px",fontSize:12,color:adminEngageResult.error?C.hard:C.easy}}>
              {adminEngageResult.error?`Error: ${adminEngageResult.error}`:(()=>{
                ...
                if(adminEngageResult.type==="reengage_test") return d.sent?`✅ Preview sent to ${d.testTo} — check your inbox`:`Failed: ${d.error||d.message||JSON.stringify(d)||"unknown error"}${d.hint?` — ${d.hint}`:""}`;
          ...
          <div key={m.topic} style={{background:C.surface,border:`1px solid ${notStarted?C.hard+"33":C.border}`,borderRadius:12,padding:"14px 16px"}}>
            ...
            {(()=>{
              ...
              if(selMods.length>0){
                ...
                const diff=minAcc>=75?"Hard":minAcc>=50?"Medium":"Easy";
