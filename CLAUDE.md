...
### Build
Cache version: `app.js?v=1800900000` (increment by 100000 before each commit)
<!-- AUTO_FACTS_END -->
...
**AI timeout scaling for exam-weight mocks (9ab0110)**: AI fetch timeout now scales dynamically with token budget (`Math.min(90000, Math.max(45000, currentMaxTokens*12))`) instead of fixed 45s. Min 45s, max 90s — prevents silent failures on large question sets (15–20 Qs). When implementing AI-dependent features with variable question counts, scale timeout proportionally to token budget.

**Exam-weight mock token budget increase (9ab0110)**: Weighted mock generation now requests `cnt*400+500` tokens (previously capped at `cnt*260+500` or 4200). When generating exam-weight mocks, expect higher token consumption per question to ensure numerically accurate and well-explained questions.
